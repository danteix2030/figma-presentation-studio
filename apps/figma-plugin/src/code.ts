...
const Command=z.discriminatedUnion('type',[z.object({type:z.literal('list'),requestId:z.string()}),z.object({type:z.literal('parse'),requestId:z.string(),id:z.string()}),z.object({type:z.literal('export-slide'),requestId:z.string(),id:z.string(),quality:z.number().min(.1).max(3).default(1)}),z.object({type:z.literal('resize'),requestId:z.string(),width:z.number().min(760).max(1800),height:z.number().min(560).max(1100)}),z.object({type:z.literal('select'),requestId:z.string(),id:z.string()}),z.object({type:z.literal('save-layer'),requestId:z.string(),id:z.string(),meta:LayerMetaSchema}),z.object({type:z.literal('save-slide'),requestId:z.string(),id:z.string(),meta:SlideMetaSchema}),z.object({type:z.literal('save-order'),requestId:z.string(),ids:z.string().array().max(2000)})]);
...
if(msg.type==='resize'){figma.ui.resize(msg.width,msg.height);result=true;}
else if(msg.type==='list'){await figma.currentPage.loadAsync();result={title:figma.root.name,order:readMeta(figma.currentPage).order??[],frames:figma.currentPage.children.filter(n=>['FRAME','COMPONENT','INSTANCE'].includes(n.type)).map((n,order)=>({id:n.id,name:n.name,x:n.x,y:n.y,width:n.width,height:n.height,order}))};}
