...
async function parse(n:SceneNode):Promise<Layer>{
 ...
 else if('exportAsync'in n){try{const bytes=await n.exportAsync({format:'PNG',constraint:{type:'SCALE',value:Math.min(2,2048/Math.max(n.width,n.height,1))}});layer.asset='data:image/png;base64,'+figma.base64Encode(bytes);if(complex)warnings.push('Grupo com máscara/blur rasterizado: '+n.name);}catch{warnings.push('Falha ao renderizar layer: '+n.name);}}
 return layer;
}
